...
description: Use when adding or modifying Hallucinate executive decisions — the leader-facing choices during the 'decision' phase (data model, API, player flow, host narration, reveal timing, round reset safety). Use PROACTIVELY whenever the user asks to add a new executive decision, change how one is presented/resolved, or touches decision.ts, LeaderDecisionPanel.tsx, or the announcement reveal flow.
