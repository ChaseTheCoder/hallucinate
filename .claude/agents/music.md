---
name: music
description: Use when implementing or maintaining Hallucinate's status-based background music (asset keying, track selection, looping, narration ducking). Use PROACTIVELY whenever the user asks to add/change music behavior, adjust ducking/fade timing, or touches useStatusMusic.ts or host audio URL conventions.
